...
OneDrive 的"按需下载"会留下占位符：`Length` 有值、修改时间也对，但文件里没有
内容，一读才去下载。几万个文件下来，导入会变成一场没有解释的通宵。

导入前先查一次，这个命令不读文件内容，也不会建数据库：

```powershell
.\.venv\Scripts\python.exe -m email_kb check-source "$env:OneDrive\EmailKnowledgeBase"
```

`ready` 为 `false` 时看 `advice`。`files_not_downloaded` 大于 0 就说明还是占位符：
在文件资源管理器里右键那个文件夹 → **始终保留在此设备上**，等同步完再跑一次。
